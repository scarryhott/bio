...
## Correct three-arm architecture

The repository currently distinguishes:

1. **Native RND1** — general diffusion-language proposal and entropy/confidence admission.
2. **Independent closure-native model** — project-derived action, independent return, and admitted-memory kernel.
3. **RND1 + closure hybrid** — RND1 proposals controlled by closure admission.

The completed 30B benchmark compared variants of arms 1 and 3. It did not evaluate arm 2 against a biological Radical Numerics model.

## Correct biological thesis comparison

The biologically relevant programme should compare:

1. the independent Bio Closure kernel;
2. an open biological model such as Evo 2;
3. optionally, the biological model plus closure return/admission.

Omnii should be treated as a reported external system until a usable public evaluation interface, weights, or reproducible benchmark package is available.

## Dataset requirement

A full closure comparison needs more than raw DNA continuation. The selected public benchmark must expose or permit a held-out returned consequence:
...
No result in this repository currently establishes superiority over Evo 2, Omnii, or another Radical Numerics biological model.
